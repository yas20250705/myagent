...
1. **Lintチェック**
   - **ruff 0.6.x**
     - Rustベースの高速リンターで複数のlintルールを統合
     - 潜在的なバグや非推奨パターンを自動検出・修正
     - 設定ファイル: `pyproject.toml`

2. **コードフォーマット**
   - **ruff format**
     - ruffに統合されたフォーマッター（Black互換）
     - Lint設定と同一ファイルで管理

3. **型チェック**
   - **mypy 1.x**
     - Pythonの静的型チェッカー
     - strict モードで厳密な型安全性を検証
     - 設定ファイル: `pyproject.toml`
...
- ruffによりESLint + Prettierに相当する機能を1ツールで提供
- pyproject.toml に設定を集約し、設定ファイルの数を最小化
